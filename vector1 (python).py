...
def comp_recall(relevance_qry_hash, vect_num):
    ...
    # The following are precisions and recalls desired is @recalls
    recalls = []
    precisions = []
    recalls.append(0)
    precisions.append(1)

    # The rank number for precisions
    num = len(ranks) - 1

    # Is Document relevent? Rank order filing
    for index in range(0,tot_number):
        if (!(res_vector[index] is None) or (relevance_qry_hash[res_vector[index]] == 1)):
            ranks.append(index + 1)
            doc_ranks.append(res_vector[index])

    # Computing below Precision Mean 1 and Precision Mean 2
    for index in range(1,num):
        recalls.append(index/num)
        precisions.append(index/ranks[index])

    PRECISION_25 = prec(0.25, recalls, precisions)
    PRECISION_50 = prec(0.50, recalls, precisions)
    PRECISION_75 = prec(0.75, recalls, precisions)
    PRECISION_100 = prec(1, recalls, precisions)

    PRECISION_MEAN_1 = (PRECISION_25 + PRECISION_50 + PRECISION_75)/3
    PRECISION_MEAN_2 = 0

    for index in range(1,11):
        PRECISION_MEAN_2 = PRECISION_MEAN_2 + prec(counter / 10, recalls, precisions)

    PRECISION_MEAN_2 = PRECISION_MEAN_2 / 10

    recall_sum = 1
    iterative_sum = num + 1
    iterative_div = num / 2
    iterative_calculation = iterative_sum * iterative_div

    for index in range(1,num):
        recall_sum = recall_sum + ranks[index]

    RECALL_NORM_CALCULATION = (1 - (recall_sum - iterative_sum) / ((tot_number - num) * num))
    # Computing below Precision Norm

    log_rank_sum = 0
    log_iterative_sum = 0

    for index in range(1,num):
        log_rank_sum = log_rank_sum + math.log(ranks[index])
        log_iterative_sum = log_iterative_sum + math.log(index)

    factorial_denominator = (tot_number * math.log(tot_number)) - ((tot_number - number) * math.log(tot_number - num)) - (num * log(num))
    PRECISION_NORM_CALCULATION = 1 - (factorial_denominator - log_iterative_sum)

    int resultList = [PRECISION_25, PRECISION_50, PRECISION_75, PRECISION_100, PRECISION_MEAN_1, PRECISION_MEAN_2, PRECISION_NORM_CALCULATION, RECALL_NORM_CALCULATION]
    return resultList
    #print "To be implemented\n"


# created PREC also
def prec(fixed_levels, sub_recs, sub_precs):
    var_prec = 0
    index = 0

    for index in range(1,len(sub_recs)-2):
        if ((sub_recs[index] <= fixed_levels) and (sub_recs[index + 1] > fixed_levels):
            var_prec = ((fixed_levels - sub_recs[index]) / (sub_recs[index + 1] - sub_recs[index]) * (sub_precs[index + 1] - sub_precs[index]) + sub_precs[index])
...
def show_relvnt(relevance_qry_hash, vect_num, my_qry_vector):
    # TODO
    global res_vector,doc_simula
    index = 0
    relevant_num = 0
    comparison = None
    # Comparison has not been used anywhere.check what to do in the case when comparison is NULL.
    print("EndOfList")
    print("************************************************************")
    print("Documents Most similar to" + comparison + "number" + vect_num)
    print("************************************************************")
    print("Similarity  Doc#  Author     Title         ")
    print("==========  ====  ======     =============================")
    print("EndOfList")
    #print "To be implemented\n"
    for counter in range(0,len(res_vector)):
        index = res_vector[index]
        if (!(index is None) and !(relevant_num[index] is None) and (relevant_num[index] == 1)):
            similarity = doc_simula[index] # Check again if a threshold perecentage of > 99.88 was needed.
            title = titles_vector[ind][:47]
...
def full_cosine_similarity(my_qry_vector, my_doc_vector, num_one, num_two):
    info = "============================================================\n"\
           "{0:15s}  {1:8d}   {2:8d}\t{3}\n"\
           "============================================================\n".format(
            "Vector Overlap", num_one, num_two, "Full Cosine similarity")
    sys.stderr.write(info)
    term = None
    weight =  0
    product = 0
    square1 = 0
    square2 = 0
    cosine_similarity_num = 0
	#$qry_vector[$qry_num]{ $word } += $tweight;
    my_query_keys = my_qry_vector.keys()
    for key in my_query_keys:
        term = key
        weight = my_query_keys[term]
        if term in my_doc_vector:
            product = product + (weight * my_doc_vector[term])
            info = "{0:15s}  {1:8d}   {2:8d}\t{3}\n".format(term, weight, my_doc_vector[term], docs_freq_hash[term])
            sys.stderr.write(info)

    query_values = my_qry_vector.keys()
    doc_values = doc_vector.keys()

    for term in query_values:
        square1  = square1 + (term * term)

    for term in doc_values:
        square2 = square2 + (term * term)

    cosine_similarity_num = product / math.sqrt(square1 * square2)

    print ("Sum of Product: " + str(product))
    print ("\n")
    print ("Sum of Squares (1st): " + str(square1))
    print ("\n")
    print ("Sum of Squares (2nd): " + str(square2))
    print ("\n")
    print ("Cosine Similarity: " + str(cosine_similarity_num))
    print ("\n")
    print ("Cosine similarity 2: " +  str(cosine_sim_a(my_qry_vector, my_doc_vector)) + "\n")
    print ("\n")
    print ("Jaccard similarity: " +  str(jaccard_sim(my_qry_vector, my_doc_vector)) + "\n")
...
def full_precision_recall_test():
    ...
    for ind in range(1,tot_queries):
